...
export function navigate(routeName: string, params?: any) {
  const performNavigation = (isRead?: boolean, unsSubscribe?: Function) => {
    if (isRead === undefined || isRead) {
      if (navigationRef.getCurrentRoute()?.name === routeName) {
        navigationRef.setParams(params)
      } else {
        navigationRef.navigate(routeName as keyof NavigatorParamList, params)
      }
      unsSubscribe?.()
    }
  }
  if (appInitObserver.isInitialized) {
    performNavigation()
  } else {
    appInitObserver.listener = performNavigation
  }
}
